...
int main( int argc, char** argv )
{
    ...
    if( face(argv,argc))
    {
            ...
            cvNamedWindow("o->okay n-not okay",CV_WINDOW_AUTOSIZE);

            cvShowImage("o->okay n-not okay",moved);
            cvSaveImage("Moved.jpg",moved);
            check1=cvWaitKey(0);
            cvDestroyWindow("o->okay n-not okay");
     }
    else
    {
	...
	cvWaitKey(0);
	cvDestroyWindow("Original Cropped");
	*/
    ...
    {   
        ...
        if( (char)c == 'w' || (char)c == ' ' || count==0 || (char)c=='b' )
        {
            if(count==0)
            {
                 // Creating a mask
            ...
            for( i = 0; i < markers2.rows; i++ )
                for( j = 0; j < markers2.cols; j++ )
                {
                    ...
                    {   
                    	...
                    	imagestore.at<Vec3b>(i,j) = Vec3b(255,255,255);
                    ...
                    double scale, bool tryflip )
{
    ...
    {   
        ...
        for( vector<Rect>::const_iterator nr = nestedObjects.begin(); nr != nestedObjects.end(); nr++ )
        {
            ...
            center.x = cvRound((r->x + nr->x + nr->width*0.5)*scale);
            center.y = cvRound((r->y + nr->y + nr->height*0.5)*scale);
            if(cvRound((nr->width + nr->height)*0.25*scale)==maxr)
                circle( img, center, radius, color, 3, 8, 0 );
        }   
        ...
        {   
            ...
            if(idx==-1)
            {
                for(i1=0;i1<3;i1++)
                    for(j1=0;j1<3;j1++)
                    {
                          b4.at<Vec3b>(i,j)+=b3.at<Vec3b>(i+i1-1,j+j1-1)/9.0;  
                    }
            }
            else
            {
                b4.at<Vec3b>(i,j)=b3.at<Vec3b>(i,j);                
